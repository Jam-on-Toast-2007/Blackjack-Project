...
- **Play as Guest** — always starts an instant guest practice session with a fresh $1,000 bankroll and the Training preset's rules, regardless of whether you have a profile — no profile is created or affected, and the hands you play aren't saved or tracked anywhere. It's the fastest way to just try the table out. If you want your progress tracked, use an existing profile or create a new one instead.
...
- If a profile's balance drops below what's needed to place the smallest bet, you'll get a pop-up letting you either delete the profile or keep it as an **inactive** profile — inactive profiles can still be opened to review their statistics, but can't be used to play any more hands until you delete them and start fresh (or fund a different profile). If you run low on funds during a guest session instead, you'll get the option to instantly restart the practice session with a fresh $1,000. Either way, you're never stuck staring at a table you can't bet on anymore.
...
- **Count Quiz** — a Questionnaire-style knowledge quiz for count strategy: true count math (running count ÷ decks remaining), running count arithmetic over card sequences, Hi-Lo tag recall, Illustrious 18 index recall and choose-all-that-apply questions, the insurance deviation index, favorable/unfavorable count concepts, and deck-remaining estimation. Complements the hands-on Deviation Drill with recall/trivia-style questions.
